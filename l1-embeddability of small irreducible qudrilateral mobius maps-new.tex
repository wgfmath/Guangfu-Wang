...
(See Figure \ref{PG}.)   If $K$  is a  a M\"{o}bius
...
us. See the graph in Figure \ref{G1}.
\begin{figure}[!htbp]
\centering
...
Let $m$ and $f$ be the numbers of  edges and faces of $(M,\Gamma)$, respectively.
If all edges of $(M,\Gamma)$ are boundary,
 we have $4f+3=2m$, a contradiction.  This completes the lemma.
\end{proof}
...
\end{lemma}
\begin{proof}
Suppose that no two edges of $C=abc$ bound a face. That is, the  three edges of $C$ are  incident with distinct faces.  From Lemma \ref{1inner}, let  $ac$ is the inner edge
...
submap in $\Gamma$. By Lemma \ref{G1}, $\Gamma$ is not
...
\end{figure}
 ...
 map $(M,\Gamma)$. The subgraph induced by $a$, $b$, $c$,
  ...
  \ref{G1}, $\Gamma_1$ is not $l_1$-embeddable. Therefore $(M,\Gamma)$
...
Let $G$ and $H$ are $l_1$-graphs. If at least one of them is bipartite, then the new graph $G\cup_e H$ by gluing $G$ and $H$ along an edge is  $l_1$-embeddable.
\end{thm}
...
maps, European J. Combin.  (2018), https://doi.org/10.1016/j.ejc.2018.02.009.
